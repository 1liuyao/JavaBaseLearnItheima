...
/*
   【线程间通信】其实就是多个线程在操作同一个资源，但是操作的动作不同。
    ...
    注意：以上方法都使用在【同步】中，因为要对持有监视器（锁）的线程操作，而只有同步才具有锁

    为什么等待唤醒方法定义在Object类中呢？
    使用等待唤醒方法时，需要使用以下格式[锁.wait()][锁.notify()]，
    也就是说等待唤醒方法被调用时，必须明确需要唤醒和等待的是持有哪个锁上的线程
    因为一个线程不可被不同锁中的线程进行唤醒，而锁可以是任意对象，所以可以被任意对象调用的方法定义在Object类中
